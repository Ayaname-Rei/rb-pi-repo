...
@dataclass
class VisionLinkConfig:
    """树莓派 ↔ 香橙派 视觉链路配置（TCP/JSON，协议见 docs/通信协议_树莓派-香橙派.md）。

    推荐直连网线 + 静态 IP（赛场 2.4G 拥挤，有线链路确定性好）：
        树莓派 eth0: 192.168.50.1   香橙派 eth0: 192.168.50.2
    视觉节点（香橙派）为 TCP 服务端，监听 vision_port。
    """
    vision_host: str = "192.168.50.2"     # 香橙派静态 IP（WiFi 方案则改其 wlan IP）
    vision_port: int = 9000
    connect_timeout_s: float = 3.0        # TCP 连接超时（短：连接失败快速暴露）
    request_timeout_s: float = 6.0        # 请求超时（长：一次 detect = 采帧+推理，
                                          # 香橙派上 0.3~1s，首帧更慢，须放宽容忍）


@dataclass
class VisionConfig:
    """视觉伺服抓取参数（相机在香橙派上，水平朝左，检测右侧紫色块对准后机械臂抓取）。

    目标像素位置 (target_u, target_v) 是「机械臂正好能抓到紫色块」时，
    紫色块中心在画面中的像素坐标（实测 1280×720 下为 940.5, 366.0）。
    相机分辨率与 imgsz 在香橙派侧 server_config.py 配置，两侧需保持一致。
    """
    ...
    conf_threshold: float = 0.5  # 检测置信度阈值（随 detect 请求发给香橙派执行）
    # ---- 以下为鲁棒性/性能参数（2026-09-23 增）----
    detect_retries: int = 5              # 单次伺服中「未检出/链路失败」的重试次数
    detect_retry_interval_s: float = 0.3 # 重试间隔（等待曝光稳定/链路恢复）
...
class MissionConfig:
    ...
    vision_link: VisionLinkConfig = field(default_factory=VisionLinkConfig)
